...
```
NEXT STEP TO BUILD:  Step 21H (Project Chat) is built, code-complete, and
                     passes lint/tsc/build. Migration 031 is written but NOT
                     YET APPLIED to live Supabase — nothing in this step has
                     been verified against real production data yet. Waiting
                     on Marvin to run 031 in the Supabase SQL editor before
                     live verification can start.
OVERALL PROGRESS:    20 of 20 original steps + Step 21A + 21A.1 + 21B + 21C +
                     21C.1 + 21D + 21E + 21F + 21G + 21H (code complete, DB
                     unapplied), all previous steps complete and verified
                     live.
...
| 21H | Project Chat | 🔄 Code complete, migration 031 NOT YET APPLIED | (uncommitted) | project_chat_messages table + RLS, ProjectChat.tsx (send/edit/soft-delete, @mentions, realtime), Chat tab for active project members, readable chat Activity entries. Blocked on Marvin applying migration 031 before live verification. |

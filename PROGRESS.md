...
```
NEXT STEP TO BUILD:  None requested yet. Steps 21A, 21A.1, 21B, and 21C are
                     all complete and fully verified live.
OVERALL PROGRESS:    20 of 20 original steps + Step 21A + 21A.1 + 21B + 21C,
                     all complete and fully verified live.
LAST COMMIT:         8cb0ab2 (Step 21B, pushed) — Step 21C not yet
                     committed/pushed, see below.
APP STATUS:          npm run lint ✅ · npx tsc --noEmit ✅ · npm run build ✅ zero errors
                     ✅ Step 21C (project activity/audit trail) built, migration 024
                     applied and verified live — 25/25 checks passing. Found and fixed a
                     real pre-existing bug: the profiles!<table>_<column>_fkey embed
                     pattern used across the app can never work (those tables' user_id
                     columns FK to auth.users, not profiles), combined with profiles RLS
                     only ever allowing self-visibility — meaning no teammate's name has
                     ever actually rendered anywhere for a second real user. Fixed the
                     RLS half (migration 024) and the query-syntax half in this step's own
                     files; flagged the 3 other pre-existing occurrences (Team page,
                     Dashboard, TaskDetail) as a separate follow-up task, not expanded here.
                     ⚠️ Open design question (not a bug, needs a product decision) — see
                     Known Issue #11: audit_events rows generated by a project's own
                     cascade-delete get project_id nulled to avoid an FK violation, but the
                     audit_events SELECT policy requires project_id IS NOT NULL, so those
                     rows become permanently invisible to everyone instead of either being
                     cleanly removed or genuinely retained/visible as history.
```
...
| 11 | **Open design question, not fixed unilaterally:** `audit_events` rows generated by a project's own cascading delete (documenting the project/its members/its tasks being deleted) have `project_id` and `workspace_id` nulled by the Step 21A fix that avoids an FK violation during cascade — but the `audit_events` SELECT policy requires `project_id IS NOT NULL`, so those rows become permanently invisible to everyone (short of the service role) instead of either being cleanly removed or genuinely retained as visible history. Found during Step 21C's live verification: 39 such rows had silently accumulated across every prior test round back through Step 21A. Confirmed all were orphaned test residue (project_id AND workspace_id both null, no live/attached rows existed) and deleted by exact id. Three legitimate product options exist — let the cascade actually delete them too, loosen the SELECT policy so deleted-project history stays visible somewhere, or accept slow invisible growth — Marvin needs to pick one; none was chosen here. | 21C | Medium | ⬜ Not fixed — awaiting product decision |

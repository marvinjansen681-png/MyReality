...
```
NEXT STEP TO BUILD:  None requested yet. Step 21H (Project Chat) is complete
                     and verified live. Since then: a real bug Marvin hit
                     live (no way to notice/act on a real incoming access
                     request from Shafica Boyce on "Regal Bay Properties")
                     was fixed — see "Hotfix — access request notification
                     + sound + badge" below. Migration 032 for that fix is
                     written but NOT YET APPLIED.
OVERALL PROGRESS:    20 of 20 original steps + Step 21A + 21A.1 + 21B + 21C +
                     21C.1 + 21D + 21E + 21F + 21G + 21H, all complete and
                     verified live. Plus one post-21H hotfix (migration 032,
                     code complete, DB unapplied).
LAST COMMIT:         (this session's commits — see hotfix notes below)
...
| 21E | Browser UAT + Release Hardening | ✅ Complete, verified live | 5a2a58b | Full real-browser collaboration workflow verified with Marvin/Shafica accounts; found + fixed 1 real bug (migration 028, applied and verified) |
| 21F | Project Command Centre + Goals | ✅ Complete, verified live | (this session) | Migration 029 applied — project_goals + project_goal_comments, task↔goal linking, Overview/Goals tabs, goal labels, readable goal audit trail; 29/29 data-layer checks + real-browser pass; found + fixed a real "New Project modal never closes" bug (stuck framer-motion AnimatePresence exit) |
| 21G | Goal Action Steps + Project Pulse | ✅ Complete, verified live | (this session) | Migration 030 applied — create_goal_action_step + convert_task_to_goal RPCs, deadline_explanations table, Action Steps quick-add under goals, Project Pulse (Needs Attention, overdue tracking, deadline reasons), SubtaskList project-task guard; 22/22 data-layer checks + real-browser pass, zero bugs found |
| 21H | Project Chat | ✅ Complete, verified live | (this session) | Migration 031 applied — project_chat_messages table + RLS, ProjectChat.tsx (send/edit/soft-delete, @mentions, realtime), Chat tab for active project members, readable chat Activity entries; 20/20 data-layer checks + real-browser pass, zero bugs found |
...
| 13 | **Found during Step 21E's real-browser UAT, not introduced by 21E:** a pending project access requester displays as "Unknown user" to the reviewing owner/manager, instead of their real name/email. `ProjectAccessRequests.tsx` already does the correct profile lookup; the RLS-backing `can_view_profile()` function (migration 024) is the actual cause — it only grants visibility once the viewer and target already share an *active* project or workspace membership, which a brand-new requester by definition doesn't have yet. This makes the approve/reject review screen effectively blind for any first-time requester (confirmed live with a real request from Shafica, not a hypothetical) — you can approve or reject, but you can't see who you're approving. Approve/Reject themselves still function correctly underneath despite the display bug. Fixed in `supabase/migrations/028_fix_access_request_profile_visibility.sql` — narrowly extends `can_view_profile()` (not the RLS policy itself) with one more case: an owner/manager can see the profile of anyone with a pending/reviewed access request on a project they manage, scoped by the existing `can_manage_project()` check. Not a blanket widening. **Applied by Marvin and re-verified live twice:** once with a lightweight spot check, then again with a full 9-point re-verification covering the real redeem_project_invite RPC flow, approve, reject, pre-approval invisibility, a negative control (zero visibility before any relationship exists), cross-user isolation, and exact-id cleanup — 24/24 real checks passed (2 apparent failures in the test script itself were misreads of correct pre-existing behavior, not product bugs — see Step 21E notes below for detail). | 21E | Medium (blind review, not a security hole) | ✅ Fixed and verified live |
...
| 2026-07-09 | Step 21E migration 028 — applied, full 9-point re-verification | Marvin applied migration 028. Ran the full checklist he specified rather than a spot check: (1) a real requester who is NOT yet an active member redeems a real invite link via the actual redeem_project_invite RPC (not a raw insert) and lands in pending status; (2) the owner correctly sees the requester's real name/email in that pending row — the fix, confirmed working; (3) Approve still works and correctly grants active membership with the requested role; (4) Reject still works (tested on a second, separate project + request, since a requester can only have one request per project) and correctly grants no membership, plus produces a real access_request_rejected notification; (5) confirmed twice — before approval and after rejection — that the requester genuinely cannot view the project; (6) confirmed the rule is NOT blanket-opened via a negative control run BEFORE any relationship existed between the two accounts at all (zero shared workspace, zero project membership) — profile visibility correctly returned empty in that state, only becoming visible once a real pending request existed; (7) cross-user isolation re-confirmed on a third, fully unrelated test project with no invite or request at all; (8) the real "Regal Bay Properties (Pty) Ltd" project and its one real task/member row were confirmed untouched before, during, and after; (9) all cleanup (3 test projects, their HARD_DELETE markers, and the notifications this flow generated) used exact ids only. **24/26 checks passed** — the 2 apparent failures were both flaws in the test script itself, not the product: one asserted a non-manager couldn't see any access-request rows on a project, but the row in question was the requester's *own* row, which the pre-existing, unrelated `user_id = auth.uid()` self-visibility clause correctly allows regardless of role; the other asserted project_members would be empty after cleanup without accounting for the real, legitimate "Regal Bay Properties" owner row that was never touched. Once corrected for those, all 24 real assertions passed. npm run lint / npx tsc --noEmit / npm run build all clean. Migration 028 does not weaken profile or access-request RLS in any way — it adds exactly one narrow, correctly-scoped visibility case and nothing else. Step 21E is now fully verified end to end, migration and all. |

| 2026-07-09 | Step 21F — Project Command Centre + Goals, code written, migration drafted (superseded — see next entry) | Built the full step per Marvin's spec: `project_goals` (title/description/status/priority/due_date/completed_at/completed_by/sort_order, status/priority CHECKs) and `project_goal_comments`, both project-id-scoped and RLS'd off the existing `can_view_project`/`can_edit_project_content`/`can_manage_project`/`can_comment_on_project` helpers — no new permission model, same one every other table in this schema uses. `tasks.goal_id` added (nullable, ON DELETE SET NULL), enforced same-project via a BEFORE INSERT/UPDATE trigger (`enforce_task_goal_same_project`) since a CHECK constraint can't do the cross-table lookup; linking/moving a task's goal rides on the task's own existing UPDATE policy (`can_edit_project_content` — owner/manager/editor), so no new task-level policy was needed, it was already exactly right. Goal create/update/complete/reopen is owner/manager/editor; archiving is further restricted to owner/manager by a trigger (`enforce_goal_rules`), the same "RLS is permissive, a trigger narrows by which column actually changed" pattern 021 already established for project archiving — completed_at/completed_by are stamped server-side, never trusted from the client. Goal comments follow task_comments' exact permission shape (commenter can post, viewer cannot) and reuse the mention pattern via a new `create_goal_mention_notifications` RPC (a straight mirror of 026's `create_mention_notifications`, just pointed at goals). Extended `log_audit_event()` (already generic enough to need zero special-casing for project_id resolution on the two new tables) with COMPLETE/REOPEN/ARCHIVE/RESTORE action labels for goal status transitions, matching how it already labels project ARCHIVE/RESTORE. UI: new Overview tab (project-progress bar = completed tasks ÷ total tasks — documented as the deliberate choice over a goal-based ratio, since goals can legitimately be empty while tasks aren't) and Goals tab (`ProjectOverview.tsx`, `ProjectGoals.tsx`) added to the project page's tab bar (Overview/Board/List/Goals/Activity); TaskDetail gained a Goal selector (disabled for commenter/viewer, same gate as Priority/Due Date); TaskCard and ListView now show a small goal-label badge on any task linked to one. Added a page-level realtime subscription for `tasks` (separate channel name from BoardView's own) so the Overview/Goals tabs — which don't otherwise have a task-focused realtime hook — stay live when a task is completed or its goal changes from any tab. Also fixed the two flagged 21E cosmetic issues: ProjectShareModal's "Recent activity" mini-feed now runs through the same `formatAuditEvent()` the main Activity tab uses instead of printing raw `entity_type action` text (now also resolves task/goal titles, not just names); the New Project modal's close-on-success code was re-checked line by line and already correctly calls `setModalOpen(false)` after a successful create — flagged for live-browser re-confirmation rather than assumed fixed, since the 21E finding was real and this session made no change to that file's control flow. **Nothing in this step has been verified against live Supabase** — migration `supabase/migrations/029_project_goals_command_centre.sql` is written, reviewed, and matches every established pattern in 018/021/026/027, but per this project's standing workflow Claude Code has no direct DB/psql access; Marvin needs to run it via the Supabase SQL editor before any of the A–V live-verification checklist can start. `npm run lint` / `npx tsc --noEmit` / `npm run build` all pass clean (21/21 routes). Not committed to git yet — held pending the live verification pass, per this project's "never commit before tests pass" rule. |
| 2026-07-10 | Step 21F — migration 029 applied, full live verification (A–AB), one real bug found + fixed | Marvin applied migration 029. Ran two passes: an automated data-layer pass driving real authenticated REST calls as both real users, then a genuine real-browser pass (actual clicks/fills in a running dev server, not just API calls) for the parts only a browser can prove.<br><br>**Method (data-layer pass):** Real sessions for Marvin and Shafica obtained via `supabase.auth.admin.generateLink` + `verifyOtp` (service role used only to mint sessions and for read-only before/after snapshots — every actual test action ran as the real user it claimed to be, RLS enforced throughout, no service-role writes). Two temporary projects created via Marvin's real session using the app's own no-`.select()`-on-insert pattern (029A21 same RETURNING-race avoidance as `page.tsx`): P1 for the main goals matrix, P2 (Shafica never added) for the same-project-link negative test (H) and cross-user isolation (Y). Every id created was tracked in a ledger and cleanup at the end used only those exact ids — project hard-delete via Marvin's owner session (cascades members/tasks/goals/comments/columns), personal task deleted explicitly, notifications deleted via each recipient's own session, and the resulting HARD_DELETE/orphan `audit_events` marker rows deleted by exact id via service role (no authenticated DELETE policy exists on `audit_events` by design — same narrow exception documented in every prior round back to 21B).<br><br>**Data-layer results — 29/29 checks passed:** A–E (owner/manager/editor can create a goal; commenter/viewer correctly rejected by RLS with the exact `project_goals` policy violation), F–G (owner/manager/editor can link a task to a goal; commenter/viewer attempts silently affect 0 rows, task's `goal_id` confirmed unchanged), H (linking a task to a goal in a different project correctly raises "A task can only be linked to a goal in the same project" from `enforce_task_goal_same_project`), I (progress went 0/3 → 2/3 → 3/3 as tasks were marked done), J–L (complete stamps `completed_at`/`completed_by` server-side to the real actor; a goal with 0 linked tasks reads 100% per the documented Overview/ProjectGoals formula; reopening clears both fields), M–N (owner and manager can archive; editor's archive attempt is rejected by the `enforce_goal_rules` trigger with its explicit message, goal status provably unchanged after), O (archived goal excluded from the `neq('status','archived')` view), P–Q (owner/manager/editor/commenter can comment; viewer's insert rejected by RLS), R (`create_goal_mention_notifications` notifies the real active member and silently skips a fabricated non-member id — confirmed via each side's own notifications query), S (`audit_events` contains real INSERT/COMPLETE/REOPEN/ARCHIVE rows for `project_goals`, an INSERT for `project_goal_comments`, and `tasks` UPDATE rows with `goal_id` changing both into and out of a goal), Y (Shafica, never added to P2, sees zero rows for it or its goals), Z (a personal task creates/reads fine with `goal_id` staying null, unaffected by the new column), V (Overview's counts are internally consistent: goals/tasks/completed/open all reconcile against raw data). Real project confirmed untouched mid-test and DB counts (projects/goals/comments/tasks/members/workspaces/workspace_members) matched the pre-test baseline exactly after cleanup.<br><br>**Method (browser pass) and a genuine finding:** Logged into the actual running app as Marvin using a real session cookie (`sb-khcpvjtphzidwzbhtayh-auth-token`, built from a `verifyOtp` session and injected via `document.cookie` to match `@supabase/ssr`'s exact browser storage format — the same "admin-generated session exchanged into a real cookie" approach 21E used, needed again here because the plain magic-link redirect flow doesn't fit this app's PKCE-oriented `/auth/callback` route). Clicked "New Project" in the real UI, filled the form, submitted. **The project was created correctly every time (confirmed via the live projects list count incrementing and a real row in the DB) — but the modal itself never closed.** This reproduced cleanly across multiple full dev-server restarts and fresh page loads (ruling out Fast-Refresh/HMR artifacts), and — critically — clicking Cancel didn't close it either, on a modal that had *already gone through one open/create cycle*. Inspected the DOM directly: the modal and its full-screen backdrop were still mounted at `opacity: 0`, and the backdrop still had `pointer-events: auto` — i.e. framer-motion's exit animation completed but `AnimatePresence` never actually unmounted the exited children, leaving an invisible, click-swallowing, full-viewport overlay sitting over the entire Projects page after the very first modal close. This is a materially worse bug than 21E's cosmetic report of "the modal stays open" — it silently blocks every subsequent real mouse click on the page (not just via the automation tooling: confirmed via computed styles, not just accessibility-tree text). **Root cause:** `<AnimatePresence>` wrapped a bare Fragment containing two sibling `motion.div`s with no `key` props — added explicit keys first (the textbook fix) and confirmed via a clean restart that it did *not* resolve it, so the actual fix applied was to drop `AnimatePresence`/exit-animation reliance for this specific modal entirely (kept the enter animations via `initial`/`animate`, switched to plain `{modalOpen && (...)}` conditional rendering, which is React's own guaranteed-correct mount/unmount path). Re-verified clean on a full server restart: real click → fill → real click submit → project created, modal heading gone from the DOM, zero leftover backdrop element, zero console errors. Files changed for this fix: `app/(app)/projects/page.tsx` only (removed unused `AnimatePresence` import, added `React` import for the keyed `Fragment`). `npm run lint` / `npx tsc --noEmit` / `npm run build` all re-confirmed clean after the fix (21/21 routes).<br><br>**Cleanup:** all 6 browser-created "STEP21F BROWSER TEST" projects hard-deleted via Marvin's real owner session (exact ids), their `audit_events` HARD_DELETE markers purged by exact id, and 2 orphaned personal-task audit rows from the data-layer Z test (missed by the first project-scoped cleanup pass, since personal tasks carry `project_id = null`) caught and removed in a final sweep. Final state confirmed byte-for-byte back to baseline: 1 project (the real one, byte-identical), 0 goals, 0 goal comments, 1 task (the real one), 1 project_members row, 2 workspaces, 2 workspace_members, 5 `audit_events` rows — all 5 belonging to the real project's genuine pre-existing history. The temporary `.claude/launch.json` `NODE_OPTIONS=--use-system-ca` TLS workaround (same sandbox-only certificate-trust issue documented in 21E) was added for this session's browser testing and reverted before finishing — confirmed zero diff against the committed version.<br><br>**Bottom line:** Migration 029 is safe and correctly applied. Every RLS/permission/trigger rule in the spec (A–S, Y, Z, V) is confirmed correct against live production data. The one real bug found (W, X's companion finding) was in application code, not the database, and is now fixed and re-verified. The real "Regal Bay Properties (Pty) Ltd" project was never touched. Step 21F is complete and safe for Marvin to use. |
| 2026-07-10 | Step 21G — Goal Action Steps + Project Pulse, code written, migration drafted, NOT YET APPLIED | Built per Marvin's spec, driven by his real-world finding that "Accreditation" should be a goal with real assignable action steps, not a hidden personal subtask. **Two new SECURITY DEFINER RPCs** in `supabase/migrations/030_goal_action_steps_project_pulse.sql`, both re-checking the exact same permission the equivalent RLS policy would (same pattern as 026's `create_mention_notifications` — atomicity, not looser access): `create_goal_action_step(project_id, goal_id, column_id, title, assignee_id, due_date, priority)` creates the task and (if an assignee is given) its `task_assignees` row in one transaction — checks `can_edit_project_content`, that the goal and column both actually belong to the project, and that the assignee is an active project member, before doing anything, so a bad assignee can't leave a half-created task; `convert_task_to_goal(task_id)` — the deliberately safer first version per the brief's own fallback option, since moving comments/subtasks/soft-deleting the original task was judged too risky for one step — creates a new goal from the task's title/priority/due_date and links the *original task itself* under that new goal (so it becomes the goal's first action step) rather than deleting or hiding anything; documented limitation: task descriptions are TipTap JSON and aren't copied into the (plain-text) goal description. **`deadline_explanations`** table added: exactly one of `task_id`/`goal_id` via a CHECK constraint, `project_id`/`created_by` always stamped server-side from the referenced task/goal (never trusted from the client, same reasoning as `task_assignees`), viewable by anyone who can view the project, insertable by owner/manager/editor/commenter (matches `can_comment_on_project` exactly), not by viewer. **UI:** `ProjectGoals.tsx` — "Tasks under this goal" renamed to "Action Steps" with a single-row inline quick-add (title, assignee dropdown scoped to real active project members, due date, priority, Add) that goes through the new RPC; each action step row now shows its due date and an "Unassigned" flag (read from the real `task_assignees`-derived `assigneesMap`, not the stale unused `assigned_to` array column). `TaskDetail.tsx` — added a "Convert to Goal" button next to the Goal selector (owner/manager/editor only) with a confirmation dialog explaining exactly what will happen; the old `SubtaskList` is now only shown for personal tasks — for any task with a `project_id`, it's replaced with "Use Goal Action Steps for assignable project work.", closing the exact gap Marvin flagged (subtasks silently created as `is_personal=true` with no project_id/goal_id/assignee, invisible to the rest of the team). `ProjectOverview.tsx` rebuilt into **Project Pulse**: an animated progress bar (framer-motion `animate` only, no `AnimatePresence`/exit dependency — deliberately avoiding the exact bug class found and fixed in 21F), a 6-stat grid (goals, action steps, overdue, my tasks, due today, due this week), a **Needs Attention** section (overdue tasks/goals without a reason, high/urgent tasks due within 3 days, unassigned action steps with a due date, goals with zero action steps), inline "Add reason" forms for overdue items (reason + optional new expected date, writing to `deadline_explanations`), and a "Deadline explanations" list showing saved reasons with who/when/new-expected-date once added. `formatAuditEvent.ts` extended for: action-step creation reads as "created action step X under goal Y" instead of a generic task-created line; `tasks` action `CONVERT_TO_GOAL` reads as "converted task X into goal Y"; a new `deadline_explanations` case reads as "explained a missed deadline for X" with the new expected date as detail. Added `canCreateGoalActionStep`, `canExplainMissedDeadline`, `canConvertTaskToGoal` to `projectPermissions.ts` (all thin wrappers around the existing `canEditProjectContent`/`canCommentOnProject`, matching what the RPCs/RLS actually enforce — no new permission tiers invented). Two new page-level realtime subscriptions added (`project_goals` and `deadline_explanations`, both scoped by `project_id`, separate channel names from every existing subscription) so Project Pulse — which reads its own page-level `goals`/`deadlineExplanations` state, not the Goals tab's local copy — stays live when a goal is edited from the Goals tab or a reason is added by another user; the existing `tasks` realtime subscription from 21F already covers action-step creation and completion. `npm run lint` / `npx tsc --noEmit` / `npm run build` all pass clean (21/21 routes). **Nothing in this step has been verified against live Supabase** — same standing constraint as every migration-adding step: Claude Code has no direct DB/psql access, so Marvin needs to apply `030_goal_action_steps_project_pulse.sql` via the Supabase SQL editor before the A–Z live-verification checklist can run. Not committed to git yet — held pending the live verification pass. |
| 2026-07-10 | Step 21G — migration 030 applied, full live verification (A–Z), zero bugs found | Marvin applied migration 030 and confirmed. Ran the same two-pass method as 21F: an automated data-layer pass driving real authenticated REST calls/RPCs as both real users, then a genuine real-browser pass for the parts only a browser can prove (Project Pulse rendering, the "Add reason" flow, the Convert-to-Goal confirmation dialog, the Subtasks guidance text).<br><br>**Method (data-layer pass):** Real sessions for Marvin and Shafica via `supabase.auth.admin.generateLink` + `verifyOtp` (service role used only to mint sessions and for read-only before/after snapshots — every actual test action, including every RPC call, ran as the real user it claimed to be). One temporary project (P1, with a "Todo" column and an "Accreditation" goal) for the main action-step/deadline matrix, one more (P2, Shafica never added) for cross-user isolation. Every id created was tracked in a ledger; cleanup used only those exact ids — project hard-delete via Marvin's owner session (cascades everything, including `deadline_explanations` via its own `ON DELETE CASCADE`), personal task deleted explicitly, and leftover `audit_events` rows tied to the test entity ids purged by exact id via service role (same narrow, documented exception as every prior round — no authenticated DELETE policy exists on `audit_events` by design).<br><br>**Data-layer results — 22/22 checks passed:** A–C (owner/manager/editor can call `create_goal_action_step` and get back a real task with `goal_id` set), D–E (commenter/viewer calls rejected with "Not authorized to create action steps on this project" — the RPC's own `can_edit_project_content` check, not a silent no-op), F (assigning during creation produced a real `task_assignees` row in the same call), G (a fabricated non-member id was rejected with "Assignee must be an active project member" — nothing was created), H–I (the new action steps carried `project_id`/`column_id`/`is_personal=false` so they're genuinely board/list-visible, and the assigned one would appear in Shafica's My Tasks), J–K (marking two action steps done moved the goal's done-count 0→2, confirmed via a direct re-read), L (`goal_id` present on the created task, which is what the existing goal-label badge already keys off), M (`convert_task_to_goal` on a plain "Accreditation" task — mirroring the real one Marvin has — created a new goal titled "Accreditation", linked the *original* task under it via `goal_id`, left `deleted_at` null, and produced exactly one `CONVERT_TO_GOAL` audit row — nothing lost, nothing silently duplicated), R/S (a commenter-role Shafica could add a deadline reason with `project_id`/`created_by` correctly server-stamped; switched to viewer and the same insert was rejected by RLS), U/V (audit rows exist for both the deadline explanation and the goal-linked task creation), W (Shafica, never added to P2, saw zero rows for it), X (a personal task's `goal_id` stayed null, unaffected by any of this). Real project confirmed byte-identical before and after; DB counts (projects/goals/tasks/project_members/deadline_explanations/workspaces/workspace_members) matched the pre-test baseline exactly post-cleanup.<br><br>**Browser pass:** Logged into the real running app as Marvin via the same session-cookie-injection method as 21F/21E (hit the identical sandbox-only TLS trust issue on the dev server — same temporary `NODE_OPTIONS=--use-system-ca` in `.claude/launch.json`, reverted before finishing, confirmed zero diff). Built a temp project with a goal and a backdated overdue action step, then drove it for real: **O** — Project Pulse showed 0% / "0 of 1 tasks completed" / Goals 0/1 / Action steps 0/1, all numerically correct for the seeded data. **P** — the Overdue stat read "1 · need attention" and Needs Attention listed the overdue step with "5d overdue" and "Unassigned". **Q** — that row showed "Reason required · Add reason" exactly as specified. **T** — clicked Add reason, typed a real reason, saved: the row moved out of "Reason required" and a new "Deadline explanations" section appeared showing the reason text, "Marvin Jansen", and the timestamp. **N** — opened the action step's own task detail: the Subtasks section showed "Use Goal Action Steps for assignable project work." instead of the old add-subtask UI, and a "Convert to Goal" button sat next to the Goal field showing "Accreditation" already selected. Clicked Convert to Goal: the confirmation dialog rendered correctly ("This creates a new goal called... The task itself is not deleted."), and Cancel closed it cleanly with zero leftover DOM/backdrop — deliberately re-verified given 21F's AnimatePresence bug, since this dialog was built from scratch this session using the same plain-conditional-rendering pattern as that fix. Zero console errors throughout. Test project hard-deleted afterward via Marvin's real session, its audit marker purged by exact id, `.claude/launch.json` reverted.<br><br>**Bottom line:** Migration 030 is safe and correctly applied. No bugs were found this round — everything in the spec (A–Z) is confirmed working against live production data, through both the API layer and a real browser. The real "Regal Bay Properties (Pty) Ltd" project — including its actual "Accreditation" task, which Marvin can now convert for real whenever he's ready — was never touched. Step 21G is complete and safe for Marvin to use. |
| 2026-07-10 | Step 21H — Project Chat, code written, migration drafted, NOT YET APPLIED | Built per Marvin's spec: a quick, project-scoped chat that sits alongside goals/tasks/Activity, not a replacement for any of them. `project_chat_messages` in `supabase/migrations/031_project_chat.sql` — `project_id`, `sender_id`, `content`, optional `reply_to_message_id` (column included per the brief's "optional if useful", but no reply-thread UI built this step — same "don't overbuild" discipline as prior steps), `edited_at`/`deleted_at`/`deleted_by` for soft-delete. RLS: view = `can_view_project` (already excludes removed members and non-members, no extra check needed), send = `can_comment_on_project` (owner/manager/editor/commenter — viewer is read-only). The UPDATE policy is deliberately permissive (sender OR owner/manager) with a trigger (`enforce_chat_message_edit_rules`) doing the real fine-grained gating — same "RLS is broad, a trigger narrows by exactly which fields changed" pattern as 021's project-archive trigger and 029's goal-archive trigger: the sender can edit their own live content (stamping `edited_at`) or soft-delete their own message; anyone else touching the row must be owner/manager AND is only allowed to flip `deleted_at` from null to a value — they can never edit someone else's content, never undelete, never touch an already-deleted message. No DELETE policy exists at all — messages are soft-deleted only, matching the same reversible-by-default decision as project archiving (021) and goals (029). `sender_id` is always server-stamped, never trusted from the client. `create_chat_mention_notifications(message_id, mentioned_user_ids)` is a straight mirror of 026's/029's mention RPCs — the author-only trust check, `is_active_project_member()` re-validation per mentioned id so a non-member can never be notified or learn the project exists, notification body "X mentioned you in project chat" per the brief's exact wording. `log_audit_event()` extended with SEND/EDIT/DELETE_MESSAGE labels for `project_chat_messages` (a no-op UPDATE, e.g. touching only `updated_at`, is explicitly skipped rather than logged as noise). **UI:** `components/projects/ProjectChat.tsx` — message list with sender name/avatar-initial/timestamp, "(edited)" label, "Message deleted" placeholder for soft-deleted rows, inline edit-in-place for the sender's own messages, a delete button shown to the sender always and to owner/manager on any message, a helper line above the input ("Use chat for quick project communication. Create or update tasks for actual work that must be tracked.") per the brief, and a realtime subscription (separate channel name, `project:{id}:chat`, from every other subscription on the page) so new/edited/deleted messages appear live. A `TaskDetail`-style "Create task from message" quick action was explicitly left as a documented TODO for Step 21I rather than built — the brief allowed this ("only build this if it is quick and safe"), and correctly scoping which column/goal a chat-derived task would land in isn't a one-line addition. **Page integration:** `app/(app)/projects/[id]/page.tsx` gained a Chat tab, gated on `projectRole !== null` (i.e. any active membership, including viewer — viewer sees the tab and can read, just can't send, matching the brief exactly) rather than any specific role. Added `canSendProjectChatMessage`/`canDeleteAnyProjectChatMessage` to `projectPermissions.ts` (thin wrappers around the existing `canCommentOnProject`/`canManageProject`, matching exactly what the RLS policies and trigger enforce — no new permission tiers invented). `npm run lint` / `npx tsc --noEmit` / `npm run build` all pass clean (21/21 routes). **Nothing in this step has been verified against live Supabase** — same standing constraint as every migration-adding step: Claude Code has no direct DB/psql access, so Marvin needs to apply `031_project_chat.sql` via the Supabase SQL editor before the A–T live-verification checklist can run. Not committed to git yet — held pending the live verification pass. |
| 2026-07-10 | Step 21H — migration 031 applied, full live verification (A–T), zero bugs found | Marvin applied migration 031 and confirmed. Ran the same two-pass method as 21F/21G: an automated data-layer pass driving real authenticated REST calls/RPCs as both real users, then a real-browser pass for send/edit/delete plus the one thing only a browser can prove — a message sent from a separate session appearing live with zero refresh.<br><br>**Method (data-layer pass):** Real sessions for Marvin and Shafica via `supabase.auth.admin.generateLink` + `verifyOtp` (service role used only to mint sessions and for read-only before/after snapshots). One temporary project (P1) with Shafica cycled through manager/editor/commenter/viewer/removed/manager for the full role matrix, one more (P2, Shafica never added) for cross-user isolation. Every id created was tracked in a ledger; cleanup used only those exact ids — project hard-delete via Marvin's owner session (cascades `project_chat_messages` via its own `ON DELETE CASCADE`), personal task deleted explicitly, notifications deleted via each recipient's own session, leftover `audit_events` rows purged by exact id via service role (same narrow, documented exception as every prior round).<br><br>**Data-layer results — 20/20 checks passed:** A–D (owner/manager/editor/commenter can all send, each message's `sender_id` correctly server-stamped to the real actor regardless of what the client sent), E (viewer read 4 real messages but her own send attempt was rejected by RLS), F (after flipping Shafica's membership to `status='removed'`, she could read zero messages and her send attempt was rejected — `can_view_project`/`can_comment_on_project` both correctly stop evaluating a removed member as a member at all), G/Q (Shafica, never added to P2, saw zero messages and zero rows for the project itself), H (the sender editing their own message produced the new content and a stamped `edited_at`), I (a sender's own soft-delete stamped `deleted_at`/`deleted_by` to themselves), J (both directions of moderation tested — owner deleting the manager's message and manager-role Shafica deleting the owner's message — both succeeded, `deleted_by` correctly attributed to whoever actually clicked delete, not the original sender), K (editor-role Shafica's attempt to delete Marvin's message affected 0 rows — confirmed by re-reading the row directly: still not deleted — the `enforce_chat_message_edit_rules` trigger's "only owner/manager may moderate" check held), L (a deleted message's row still carries its real `deleted_at`, which is what the UI's "Message deleted" placeholder keys off), N (mentioning Marvin's real user id via `create_chat_mention_notifications` produced exactly one new `mention` notification for him), O (mentioning a fabricated non-member id produced zero notifications for that id — the RPC's `is_active_project_member` re-check held), P (`audit_events` contains real SEND/EDIT/DELETE_MESSAGE rows for the test project — 9 total across the round), R (a personal task still creates/reads fine, unaffected). Real project confirmed byte-identical before and after; DB counts (projects/chat messages/tasks/project_members/workspaces/workspace_members) matched the pre-test baseline exactly post-cleanup.<br><br>**Browser pass — item M and general sanity:** Logged into the real running app as Marvin via the same session-cookie-injection method as every prior round (hit the identical sandbox-only TLS trust issue on the dev server — same temporary `NODE_OPTIONS=--use-system-ca` in `.claude/launch.json`, reverted before finishing, confirmed zero diff). Opened a temp project's real Chat tab: empty state and the "Use chat for quick project communication..." helper line both rendered correctly. Sent a real message through the actual input + Send button — appeared instantly with sender name and relative timestamp. **M** — sent a second message from a completely separate script-driven session (not the browser's own client instance, so this genuinely exercises the realtime channel rather than local state) and confirmed it appeared in the open browser tab with zero page reload. Then, still in the same browser session: clicked the hover-revealed edit button on the sender's own message, changed its text, hit Enter — content updated in place and an "(edited)" label appeared. Clicked delete on that same message — it correctly flipped to the "Message deleted" placeholder. Zero console errors throughout. Test project hard-deleted afterward via Marvin's real session, its audit marker purged by exact id, `.claude/launch.json` reverted (confirmed zero diff).<br><br>**Bottom line:** Migration 031 is safe and correctly applied. No bugs were found this round — everything in the spec (A–T) is confirmed working against live production data, through both the API layer and a real browser, including the realtime delivery path that only a genuine second session could prove. The real "Regal Bay Properties (Pty) Ltd" project was never touched. Step 21H is complete and safe for Marvin to use. |
| 2026-07-13 | Hotfix — access request notification + sound + Share badge, code written, migration drafted, NOT YET APPLIED | Marvin hit this live: Shafica Boyce requested access to the real "Regal Bay Properties (Pty) Ltd" project, and he had no way to notice or act on it — no notification arrived, and he couldn't find an Approve/Reject control anywhere. Investigated: the Approve/Reject UI has actually existed since Step 21B (`ProjectAccessRequests.tsx`, fully functional, correctly gated to owner/manager) — it just lives inside Share > Requests, and nothing pointed him there. **Immediate workaround (no code change needed): Share button (top right of the project page) > Requests tab > Approve/Reject — this already works today.** The real gap was notification + discoverability, not the approval mechanism itself: `notify_access_request_status()` (026) only ever notified the *requester* once a decision was made; nothing notified the *reviewer* that a decision was needed in the first place. Fixed in `supabase/migrations/032_notify_new_access_request.sql` — a new `notify_new_access_request()` trigger fires on every `project_access_requests` INSERT and notifies every active owner/manager on that project (not just the creator — a manager can approve too, matching `can_manage_project()`'s treatment everywhere else in this schema) with "X requested access to \"Project\"" and a link straight to the project. Added `access_request_received` to the `notifications.type` CHECK constraint (same explicit-name pattern 026 used, so this migration can reference `notifications_type_check` directly rather than looking it up). **Notification sound:** Marvin asked for a sound when notifications come through — added `lib/utils/notificationSound.ts`, a short two-tone chime synthesized with the Web Audio API (no audio file to source, host, or commit) — wired into `useNotifications.ts`'s existing realtime INSERT handler, so it plays for every notification type, not just this new one, matching how the bell already treats all notification types uniformly. **Share button badge:** added a live pending-request-count badge on the project page's Share button itself (`app/(app)/projects/[id]/page.tsx`) — gold circle, same visual language as the header's unread-count badge — so a reviewer sees there's something waiting without having to think to open Share at all; backed by a project-scoped realtime subscription on `project_access_requests` (`*` event, so it also clears live the moment a request is approved/rejected from any tab). Updated `types/index.ts` (`NotificationType` gains `access_request_received`) and `components/layout/Header.tsx` (icon mapping, 📨). `npm run lint` / `npx tsc --noEmit` / `npm run build` all pass clean (21/21 routes). The badge and sound work immediately — they don't depend on the new migration. Only the new "someone requested access" notification itself needs `032_notify_new_access_request.sql` applied via the Supabase SQL editor before it starts firing. Not committed to git yet — held pending Marvin's confirmation this resolves what he saw. |

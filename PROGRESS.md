...
```
NEXT STEP TO BUILD:  Step 21G (Goal Action Steps + Project Pulse) is built,
                     code-complete, and passes lint/tsc/build. Migration 030
                     is written but NOT YET APPLIED to live Supabase —
                     nothing in this step has been verified against real
                     production data yet. Waiting on Marvin to run 030 in
                     the Supabase SQL editor before live verification can
                     start.
OVERALL PROGRESS:    20 of 20 original steps + Step 21A + 21A.1 + 21B + 21C +
                     21C.1 + 21D + 21E + 21F + 21G (code complete, DB
...
| 21G | Goal Action Steps + Project Pulse | 🔄 Code complete, migration 030 NOT YET APPLIED | (uncommitted) | create_goal_action_step + convert_task_to_goal RPCs, deadline_explanations table, Action Steps quick-add under goals, Project Pulse (Needs Attention, overdue tracking, deadline reasons), SubtaskList project-task guard. Blocked on Marvin applying migration 030 before live verification. |
